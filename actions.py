...
def get_botgal():
    ...
    # goes to botgal
    line_follow(ROBOT.choose(red=3000, blue=2550, yellow=2750, green=0))
    drive_straight(ROBOT.choose(red=530, blue=620, yellow=420, green=0))
    # backs up to prepare for grab
    drive_straight(ROBOT.choose(red=100, blue=100, yellow=200, green=0), -1)
...
def deliver_botgal():
    ...
    drive_straight(ROBOT.choose(red=600, blue=600, yellow=600, green=0), -1)
    # turns left past black linedrive_straight_until_white
    gyro_turn(0, 100, ROBOT.choose(red=40, blue=40, yellow=40, green=0))
    # turns left to next black line
    drive_until_black(-85, 85, False)
    # line follows to botgal delivery zone
    line_follow_right(ROBOT.choose(red=1150, blue=850, yellow=1000, green=0))
    ...
    drive_straight(ROBOT.choose(red=350, blue=350, yellow=350, green=0), -1)
...
def get_wire_shark():
    ...
    drive(-65, 65, ROBOT.choose(red=160, blue=140, yellow=130, green=0))
    # lifts backclaw again to prepare to get wireshark
    move_servo_lego(BackClaw.UP, 2)
    # backs up to wireshark
    drive_straight(ROBOT.choose(red=2900, blue=2700, yellow=2400, green=0), -0.5)
    drive_straight(ROBOT.choose(red=0, blue=20, yellow=0, green=0))
...
def ws_to_ddos():
    ...
    drive_straight(ROBOT.choose(red=400, blue=400, yellow=400, green=0))
    # moves backclaw farther down to more securely grab wireshark
    drive_straight(ROBOT.choose(red=150, blue=150, yellow=150, green=0), -1)
    move_servo_lego(BackClaw.SUPERDOWN, 1)
    enable_servo(BackClaw.port)
    drive_straight(ROBOT.choose(red=150, blue=150, yellow=150, green=0), -1)
    # line follow to ddos
    line_follow_ticks(ROBOT.choose(red=14350, blue=14100, yellow=14350, green=0))
    # turns left past black line
    drive(-85, 85, ROBOT.choose(red=1450, blue=1450, yellow=1450, green=0))
    # turns until next black line
    drive_until_black(-80, 80, False)
    # turns until the end of the black line
    drive_until_white(-40, 40, False)
    # turns left to line up with ddos
    drive(-65, 65, ROBOT.choose(red=155, blue=135, yellow=100, green=0))
    # backs up to position wireshark under ddos
    drive_straight(ROBOT.choose(red=850, blue=800, yellow=830, green=0), -1)
    stop_motors()


def ddos_to_analysis():
    # shakes to disperse ping-pongs in wireshark
    drive(65, 65, ROBOT.choose(red=200, blue=100, yellow=100, green=0))
    msleep(50)
    drive(-65, -65, ROBOT.choose(red=200, blue=100, yellow=100, green=0))
    msleep(50)
    drive(-65, 65, ROBOT.choose(red=200, blue=200, yellow=100, green=0))
    msleep(50)
    drive(65, -65, ROBOT.choose(red=200, blue=200, yellow=100, green=0))
    msleep(50)
    drive(-65, 65, ROBOT.choose(red=200, blue=200, yellow=100, green=0))
    ...
    dramatic_line_follow(ROBOT.choose(red=1500, blue=1450, yellow=1500, green=0))
    # turns left to line up with delivery zone
    drive(0, 85, ROBOT.choose(red=1500, blue=1500, yellow=1500, green=0))
    # backs up to put wireshark in delivery zone
    drive_straight(ROBOT.choose(red=850, blue=850, yellow=750, green=0), -1)
    # releases wireshark
...
def knock_over_rings():
    ...
    drive_straight(ROBOT.choose(red=900, blue=800, yellow=900, green=0))
    # turns right to prepare to knock over rings
    drive(-80, 80, ROBOT.choose(red=1650, blue=1450, yellow=1650, green=0))
    # lowers the arm to prepare to knock over rings
    move_servo_lego(Arm.RING, 4)
    # turns quickly to knock over rings
    drive(-100, 100, ROBOT.choose(red=500, blue=500, yellow=450, green=0))
    stop_motors()
...
def get_noodle_one():
    ...
    line_follow_left(ROBOT.choose(red=500, blue=350, yellow=500, green=0))
    # turns left to face the noodle
    drive_until_white(-80, 80, False)
    gyro_turn(-80, 80, ROBOT.choose(red=88, blue=89, yellow=87, green=0))
    # drives forward so the noodle is within reach
    drive_straight(ROBOT.choose(red=700, blue=650, yellow=750, green=0))
    ...
    drive_straight(ROBOT.choose(red=400, blue=400, yellow=400, green=0), -1)
    # releases the claw
    move_servo_lego(Claw.NOODLE_OPEN, 2)
    # moves the arm and drives forward to more securely grab the noodle
    move_servo_lego(Arm.RED_NOODLE_GRAB_2, 4, False)
    drive_straight(ROBOT.choose(red=400, blue=300, yellow=200, green=0))
    # grabs the noodle
...
def deliver_noodle_one():
    ...
    drive_straight(ROBOT.choose(red=1200, blue=1050, yellow=800, green=0), -1)
    # lifts the arm
    move_servo_lego(Arm.STRAIGHT, 4)
    # drives away from analysis lab to avoid hitting the cubes while turning
    drive_straight(ROBOT.choose(red=800, blue=1200, yellow=800, green=0), 1)
    # turns around to score the noodle
    gyro_turn(-100, 100, ROBOT.choose(red=145, blue=155, yellow=145, green=0))
    drive_straight(ROBOT.choose(red=600, blue=600, yellow=600, green=0), -1)
...
def yellow_get_noodle_one():
    # waits for Create
    msleep(ROBOT.choose(red=13000, blue=13000, yellow=13000, green=0))
    # moves arm up so it does not run into anything
    move_servo_lego(Arm.STRAIGHT, 4, False)
    # turns left past the black line
    drive_until_black(-60, 60, False)
    drive_until_white(-60, 60, False)
    # line follows to line up with the noodle
    line_follow_left(ROBOT.choose(red=700, blue=350, yellow=150, green=0))
    # turns left to face the noodle
    drive_until_white(-80, 80, False)
    gyro_turn(-80, 80, ROBOT.choose(red=90, blue=90, yellow=90, green=0))
    # drives forward so the noodle is within reach
    wait_for_button()
    drive_straight(ROBOT.choose(red=1800, blue=1800, yellow=1850, green=0))
    wait_for_button()
    stop_motors(0)
...
def yellow_deliver_noodle_one():
    # removes the noodle from the server rack
    drive_straight(ROBOT.choose(red=1200, blue=1050, yellow=1500, green=0), -1)
    # turns around to score the noodle
    drive(-80, 80, ROBOT.choose(red=1200, blue=1570, yellow=1550, green=0))
    drive_straight(ROBOT.choose(red=500, blue=500, yellow=500, green=0))
    stop_motors(0)
    motor_power(SERVO_REPLACEMENT, -100)
    msleep(200)
    stop_motors()
...
def avoid_create():
    gyro_turn(100, -100, ROBOT.choose(red=75, blue=65, yellow=65, green=0))
    drive_straight(ROBOT.choose(red=2500, blue=2500, yellow=2500, green=0))
    stop_motors()
